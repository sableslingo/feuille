%%% ---------------
%%% PREAMBLE
%%% ---------------
\documentclass[french,11pt]{article}

% Define geometry (without using the geometry package)
\usepackage[a4paper]{geometry}
\geometry{landscape, twocolumn, textwidth=28.2cm, textheight=19.5cm, columnsep=16mm}

%\frenchspacing						% better looking spacing

% Call packages we'll need
\usepackage{graphicx}				% images
\usepackage{multicol}
\usepackage{multirow}
\usepackage{url}					% clickable links
\usepackage{marvosym}				% symbols
\usepackage{wrapfig}				% wrapping text around figures
\usepackage{fontspec}			% font encoding
\usepackage{xunicode}
\usepackage{phonenumbers}
\usepackage[hidelinks]{hyperref}
\usepackage{ragged2e}
\usepackage{pst-barcode}
\usepackage{titlesec}
\usepackage{framed}
%\usepackage[default]{raleway}
\usepackage{tocvsec2}
% Customize (header and) footer
\usepackage{fancyhdr}
\usepackage{enumitem}
\usepackage{tabularx}
\usepackage{fontawesome}
\usepackage{lipsum}
\usepackage{babel}
%\usepackage{currency}
%\pagestyle{fancy}
\pagestyle{empty}
\setmainfont{Carlito}
\usepackage{caption}
\captionsetup[table]{labelformat=empty}
\setlist{nosep,noitemsep}

%\newfontfamily\headingfont[]{Arial}
%\titleformat*{\section}{\Large\bfseries\sffamily}
%\titleformat*{\section}{\Large\headingfont}

%\renewcommand{\headrulewidth}{0.0pt}	% no bar on top of page
%\renewcommand{\footrulewidth}{0.4pt}	% bar on bottom of page

%%% ---------------
%%% DEFINITIONS
%%% ---------------

% Define separators

% Define Title en News input
\newcommand{\JournalName}[1]{%
		\begin{center}
			%\Huge \usefont{T1}{augie}{m}{n}
            \Large \usefont{T1}{augie}{m}{n}
			#1%
		\end{center}
		\par \normalsize \normalfont}

\newcommand*{\chants}{../chants}
%\newcommand*{\messe}{../messe_grace}
\newcommand*{\messe}{../messe_soleil_des_nations}
\newcommand*{\pu}{../pu}
\newcommand*{\psaumes}{../psaumes}
\newcommand*{\footer}{..}

%\DefineCurrency{EUR}{name={euro}, plural={euros}, symbol={\euro}, iso={EUR}, kind=iso}

\newcommand{\presbytere}{\textbf{Presbytère}}
\newcommand{\sjb}{\textbf{St Jean-Baptiste}}
\newcommand{\sx}{\textbf{Ste Croix}}
\newcommand{\landsberg}{\textbf{Landsberg}}

\newcommand{\NewsItem}[1]{%
\vspace{3pt}
\underline{\textbf{#1}}
	%	%\usefont{T1}{augie}{m}{n}
	%	\large \textbf{#1} %\vspace{3pt}
   %     %\Large #1 \vspace{4pt}
	%	%\par
   %     \normalsize \normalfont
		  }

\newcommand{\NewsAuthor}[1]{%
			\hfill by \textsc{#1} \vspace{4pt}
			\par \normalfont}
%\sisetup{locale=FR}
%\sisetup{group-minimum-digits=3}

\graphicspath{{../images/}}

%pas de numérotation des sections
\setsecnumdepth{none}
\setlength{\parindent}{0pt}
%%% ---------------
%%% BEGIN DOCUMENT
%%% ---------------
\begin{document}

\NewsItem{CHANT D'ENTRÉE}\\
	\input{\chants/fais_nous_marcher}

\NewsItem{CHANT}\\
	\input{\chants/comme_un_veilleur}

\NewsItem{PRÉPARATION PÉNITENTIELLE}\\
	\input{\messe/kyrie}

%\NewsItem{GLORIA}\\
%	\input{\messe/gloria}


% -----
\NewsItem{1\iere{} LECTURE} Is 2, 1-5
% -----

\NewsItem{PSAUME}
\input{\psaumes/psaume_121a}

% -----
\NewsItem{2\ieme{} LECTURE} Rm 13, 11-14a

\NewsItem{ACCLAMATION}\\
%\input{\messe/alleluia}
Réjouis toi Jérusalem Alléluia Alléluia Voici qu’il vient l’Emmanuel.

\NewsItem{ÉVANGILE} Mt 24, 37-44

\NewsItem{HOMÉLIE}

\NewsItem{PROFESSION DE FOI}
%\\ \input{\chants/je_crois_en_toi}

%\newpage

\NewsItem{PRIÈRES UNIVERSELLES}
\input{\pu/viens_nous_sauver}
%\input{\pu/souviens_toi}
%\input{\pu/seigneur_donne}
%\input{\pu/ecoute_la_priere}
%\input{\pu/seigneur_ecoute}
%\input{\pu/dans_ton_royaume}
%\input{\pu/entends_nos_prieres}
%\input{\pu/accueille_au_creux}
%\input{\pu/par_ta_sainte_mere}

\NewsItem{OFFERTOIRE}

\NewsItem{PRIÈRES SUR LES OFFRANDES}\\
\textit{Nous nous levons et nous répondons : }
Que le Seigneur reçoive de vos mains ce sacrifice à la louange et à la gloire
de Son nom, pour notre bien et celui de toute l’Église.


\NewsItem{SANCTUS}
\input{\messe/sanctus}

\NewsItem{ANAMNÈSE}\\
\input{\messe/anamnese}

\NewsItem{NOTRE PÈRE}

\NewsItem{AGNUS} 
\input{\messe/agnus}

\NewsItem{COMMUNION}\\
%\NewsItem{COMMUNION} \textbf{orgue (samedi) - chant (dimanche)} \\
\input{\chants/quand_le_seigneur_se_montrera}

\NewsItem{ANNONCES PAROISSIALES}


\NewsItem{CHANT D'ENVOI}\\
\input{\chants/aube_nouvelle}

\newpage

\NewsItem{Intentions de messe}
\begin{itemize}
\item[\Cross] Famille GENNETAY -SOULARD  (\sx)
\item[\Cross] Jean et Angélique SCHITTER (11h00)
\item[\Cross] Mme Augustine MAETZ(11h00)
\end{itemize}

\begin{table}[!h]
\centering
\caption{Informations paroissiales}
\begin{tabularx}{\columnwidth}{|p{0.1\columnwidth}p{0.1\columnwidth}|p{0.2\columnwidth}X|}
\hline
%Mardi &  18 nov.  & \multicolumn{2}{c|}{ \emph{S. Martin de Tours, évêque} } \\
Mardi & 02 déc. & \sjb & Vêpres 18h15 \\
& &      & Messe 18h30 \\
\hline
Mercredi &  03 déc.  & \multicolumn{2}{c|}{ \emph{S. François Xavier, prêtre} } \\
& & \sx		& Messe 09h00 \\
%Mercredi & 26 nov. & \sx & Messe 09h00 \\
\hline
Jeudi &  04 déc. &		\presbytere & Confessions 16h00 - 17h30 \\
&   &		\sjb &Salut au Saint Sacrement 18h15 \\
& &		&Messe 18h30 \\
\hline
%Vendredi &  28 nov.  & \multicolumn{2}{c|}{ \emph{Présentation de la Bienheureuse Vierge Marie} } \\
Vendredi &  05 déc.  & 	\sjb & Laudes 08h45 \\
%& & \sjb		& Laudes 08h45 \\
& &		& Messe 09h00 \\
\hline
Samedi &  06 déc.  & 	\sjb & Messe anticipée 18h00 \\
\hline
Dimanche &  07 déc.  & \multicolumn{2}{c|}{ \emph{2\ieme{} Dimanche de l'Avent} } \\
%Dimanche &  16 nov.  &		\sx & Messe 09h30 \\
& &		\sx & Messe 09h30 \\
& &		\sjb & Messe 11h00 \textbf{(Éveil à la foi)} \\
\hline
\end{tabularx}
\end{table}

%\begin{framed}
\begin{itemize}
\item
\textbf{Collecte de jouets} en faveur de l'association Carijou le samedi 13 déc. de 14h à 17h au local scout du groupe Bayard, 18 rue de Geispolsheim à Lingolsheim.
\item
Concert et conte de Noël \textbf{Lingorchestra} dim. 14 déc. à 16h00 à \sjb.
\item
Concert \textbf{Glorious} dim. 14 déc. au Zénith de Strasbourg à 18h00. 35~€ par personne. La paroisse prend en charge 10~€ pour les collégiens et lycéens. Contact Nathalie ou Louise \texttt{nathalie\_dick@yahoo.fr} jusqu'au 31 oct.
\item
\textbf{Concert} avec la chorale CHORENSTRA et la chorale des hommes de Molsheim le dim. 21 déc. à 17h00 à \sjb.
\item
\textbf{Confessions} au presbytère : les 1\iers{} jeudis du mois de 16h à 17h30. Les 3\iemes{} samedis du mois de 15h à 16h30.
\end{itemize}
%\end{framed}

\NewsItem{Répétitions des chorales}
\begin{description}
\item[Chorales paroissiales] : vendredi 20h15 à Sainte Croix
\end{description}

\input{\footer/footer}

\newpage

\JournalName{Communauté de Paroisses de Lingolsheim 
%\\ \normalsize \textit{Notre Dame des Sables}
%\\ \large \'{E}glise Saint Jean-Baptiste
\\  \normalsize \textit{1\ier{} Dimanche de l'Avent - A}
- \large Dimanche 30 novembre 2025}
%\\ \large Dimanche 30 novembre 2025}
%\noindent\HorRule{3pt} \\[-0.75\baselineskip]
%\HorRule{1pt}
% -----

% Front article
% -----
%\vspace{0.5cm}
%	\SepRule
%\vspace{0.5cm}

%\begin{center}
\begin{minipage}[h]{1.0\linewidth}
\setlength{\parindent}{1em}
\input{edito}
\end{minipage}
%\end{center}
% -----
\end{document}
